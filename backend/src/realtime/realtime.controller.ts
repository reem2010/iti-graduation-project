...
export class RealtimeController {
  ...
  @UseGuards(JwtAuthGuard)
  @ApiExcludeEndpoint()
  async testDoctorVerified(@Request() req, @Body() body: { doctorId: number }) {
    const notification = await this.notificationFacade.notifyDoctorVerified(
      body.doctorId,
    );
  ...
  @UseGuards(JwtAuthGuard)
  @ApiExcludeEndpoint()
  async testWelcome(@Request() req, @Body() body: { userId: number }) {
    const notification = await this.notificationFacade.notifyWelcome(
      body.userId,
    );
  ...
  async testAppointmentCancelled(
    ...
    const notification =
      await this.notificationFacade.notifyAppointmentCancelled(
        body.patientId,
        body.doctorId,
        body.appointmentId,
        new Date(body.date),
  ...
  async testNewMessage(
    @Request() req,
    @Body()
    body: { recipientId: number; senderName: string; messageId: number },
  ) {
