...
import { DoctorProfile, UpdateDoctorProfileDto } from "@/types";
...
export default function DoctorProfileDetails() {
  ...
  const [currentProfileForm, setCurrentProfileForm] =
    useState<UpdateDoctorProfileDto>({
      title: "",
      specialization: "",
      yearsOfExperience: 0,
      consultationFee: 0,
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        setError(null);
        const profileRes = await doctorProfileApi.getDoctorProfile();
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const updatedProfile = await doctorProfileApi.updateDoctorProfile(
        cleanedForm
      );
  ...
  const handleProfileChange = (
    ...
    setCurrentProfileForm(updatedForm);
  };
      ...
      <section className="bg-green-50 p-6 rounded-lg shadow-sm">
        ...
        {isEditingProfile ? (
          <form onSubmit={handleProfileSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                value={
                  currentProfileForm.consultationFee !== undefined
                    ? currentProfileForm.consultationFee
                    : ""
                }
                onChange={handleProfileChange}
                className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500"
            ...
            <div className="flex items-center">
              ...
              />
              <label
                htmlFor="isAcceptingNewPatients"
          ...
          <div className="space-y-2 text-gray-700">
            ...
            <p>
              <strong className="font-medium">Consultation Fee:</strong> $
            ...
            <p>
              <strong className="font-medium">Accepting New Patients:</strong>{" "}
              {doctorProfile.isAcceptingNewPatients ? "Yes" : "No"}
            </p>
          </div>
        ) : (
